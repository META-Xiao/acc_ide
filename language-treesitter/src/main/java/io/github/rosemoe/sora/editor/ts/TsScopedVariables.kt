...
import io.github.rosemoe.sora.editor.ts.adapter.TSQueryCaptureAdapter
 ...
 */
class TsScopedVariables(tree: TSTreeAdapter, text: UTF16StringAdapter, val spec: TsLanguageSpec) {

    private val rootScope: Scope = Scope(0, tree.rootNode.endByte / 2)

    init {
        if (spec.localsDefinitionIndices.isNotEmpty()) {
            TSQueryCursorAdapter.create().use { cursor ->
                cursor.exec(spec.tsQuery, tree.rootNode)
                var match = cursor.nextMatch()
                val captures = mutableListOf<TSQueryCaptureAdapter>()
                while (match != null) {
                ...
                var lastAddedVariableNode: TSNodeAdapter? = null
