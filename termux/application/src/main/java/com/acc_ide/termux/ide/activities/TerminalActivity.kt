...
package com.acc_ide.termux.ide.activities
...
import com.acc_ide.termux.ide.terminal.IdeTerminalSessionClient
import com.acc_ide.termux.ide.terminal.IdesetupSession
import com.acc_ide.termux.ide.utils.Environment
import com.acc_ide.termux.ide.utils.flashError
...
class TerminalActivity : TermuxActivity() {
  ...
  // Add Kotlin properties that delegate to Java getters
  protected val termuxService: TermuxService?
    get() = getTermuxService()
  
  protected val termuxTerminalSessionClient: TermuxTerminalSessionActivityClient
    get() = getTermuxTerminalSessionClient()
  ...
  override fun onCreateNewSession(
    ...
    if (canAddNewSessions) {
      ...
      this.flashError(R.string.msg_terminal_new_sessions_disabled)
    }
  }
  ...
  private fun addIdesetupSession(args: Array<String>) {
    val script = IdesetupSession.createScript(this) ?: run {
      ...
      this.flashError(R.string.msg_cannot_create_terminal_session)
      return
    }

    Log.d("IdeSetupConfig", "buildIdeSetupArguments: ${args.joinToString(separator = " ")}")

    val session = IdesetupSession.wrap(
